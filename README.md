...
使用此 toolkit 前，需要先完成以下两步：

1. **VPN 连通**——通过 UniVPN 或其他公司指定的 VPN 客户端连接到内网
2. **SSH 配置**——手动登录一次服务器，确保 SSH 公钥已配置好，后续 AI 才能通过 SSH 操作

这两步完成后再安装 toolkit。如果过程中遇到问题，可参考蓝区服务器环境指南。
