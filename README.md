...
安装 toolkit 本身不需要任何环境依赖。但在 AI 实际连接服务器之前，需要确保：

1. **VPN 已连通**——通过公司指定的 VPN 客户端连接到内网
2. **SSH 公钥已配置**——手动登录一次服务器，确保 AI 后续可以通过 SSH 免密操作

AI 在首次激活时会引导你完成服务器配置。如果连接遇到问题，可先检查 VPN 和 SSH 配置。
...
**自动触发**——正常和 AI 聊天即可，提到"服务器""SSH""NPU""容器"等关键词时，AI 会自动匹配此 skill。

**显式指定**（Claude Code）——安装后使用 `/blue-server-toolkit` 命令强制激活。其他 Agent 工具的显式调用方式请查阅对应官方文档。

首次激活时，AI 会自动安装辅助脚本并引导你填写服务器配置，完成后即可开始使用。
...
配置文件位置：`~/.blue_server_toolkit/config.json`
